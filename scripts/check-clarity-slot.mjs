...
if (eventName !== "schedule" && !watchdogTarget) {
  ...
  console.log(JSON.stringify({ shouldPost: true, reason: "manual_run" }, null, 2));
} else {
  const target = resolveRunTarget({ eventName, watchdogTarget });
  if (!target) throw new Error("Could not resolve the current CLARITY Act schedule target");
  ...
  const runs = await github.listWorkflowRuns(CLARITY_ACT_WORKFLOW);
