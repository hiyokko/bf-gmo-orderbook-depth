...
try {
  ...
  console.error(`Orderbook depth failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
}

async function main() {
  const options = parseArguments(process.argv.slice(2));
  const outputPath = path.join(repositoryRoot, "output", "latest.json");
  if (!options.dryRun) {
    await loadEnvFile(path.join(repositoryRoot, ".env"));
  }
  ...
  console.log(`Fetched at: ${result.report.fetchedAtJst} JST`);
  console.log(
    options.dryRun
