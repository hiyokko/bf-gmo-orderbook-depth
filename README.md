...
通常の板表示に合わせ、ask側（成行BUY）を `3 → 1 → 0.5 → 0.3 → 0.1 → best ask`、MIDを挟んでbid側（成行SELL）を `best bid → 0.1 → 0.3 → 0.5 → 1 → 3` の順に表示します。best行のprice impactもmidからの距離です。bitFlyerとGMOコインの表示ブロック間には空行を1行入れます。
